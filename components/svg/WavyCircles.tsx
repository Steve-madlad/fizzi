...
export function WavyCircles({
  notAnimated,
  ...props
}: SVGProps<SVGSVGElement> & { notAnimated?: boolean }) {
  const svgRef = useRef<SVGSVGElement>(null);
  useGSAP(() => {
    if (notAnimated || !svgRef.current) return;

    const inner = svgRef.current.querySelector('.wavy-circles-inner');
    const outer = svgRef.current.querySelector('.wavy-circles-outer');
